...
- **Offline-first:** ноль сети для основной работы; озвучка — системный
  скринридер (on-device); без телеметрии без явного opt-in.
...
- Поток детекта: connect → **Identity Request** → Identity Reply содержит Model ID
  **и версию прошивки** → `registry.match(model_id)` → `DeviceProfile` →
  `profile.firmware_support(version)` (см. §3.4).
...
### 3.4 Совместимость прошивки (firmware)
Версию прошивки получаем **из транспорта** — это 4-байтное поле в Universal
Identity Reply (`F0 7E dd 06 02 … vv vv vv vv F7`); codec уже отдаёт его как
`IdentityReply.version`. Отдельный запрос не нужен.

- `device::FirmwareVersion([u8;4])` — сравнимая (Ord по кортежу) + `display()`.
  Точный маппинг 4 байт → «1.05» **verify on HW** (`version_format` в профиле);
  пока — best-effort.
- Профиль перечисляет **протестированные** версии (`firmware.tested`). Политика
  `profile.firmware_support(version) -> FirmwareSupport`:
  - `Tested` — версия в списке протестированных;
  - `UntestedNewer` — новее максимальной протестированной (вышла новая прошивка);
  - `UntestedOlder` — старее минимальной;
  - `Unknown` — не распарсить / список пуст.
- **Политика — НИКОГДА не блокируем:** при `≠ Tested` приложение **работает как
  обычно**, но при коннекте **проговаривает** несовместимость (High-priority
  анонс) и держит это в «Device info» для повторного запроса. Пример: «Подключено:
  V31, прошивка 1.10. Эта прошивка новее протестированных с Tactus — должно
  работать; сообщите о проблемах.»

См. [ADR-0009](adr/0009-firmware-compatibility-policy.md).

### 3.5 Конкретно: как поддержан V31 (end-to-end)
Главное: **поддержка конкретного модуля — это данные, а не код.** В коде нет и не
будет «модуля V31». Есть generic-код + файл-описание модуля. Аналогия (для фронта):
как OpenAPI/JSON-схема описывает REST API, а один generic-клиент по ней работает —
так `roland-v31.json` описывает V31, а наш код его читает.

Где что лежит:
```
core/crates/            generic, НЕ знает ни одного конкретного модуля
  sysex/                механика протокола (RQ1/DT1/checksum/кодировки)  [готово]
  device/               грузит профиль, выбирает по Model ID, резолвит адреса
  engine/               работает с модулем через активный профиль
profiles/               ЕДИНСТВЕННОЕ место с V31-спецификой (ДАННЫЕ)
  roland-v31.json       адреса, кодировки, диапазоны, capabilities, firmware
  catalogs/roland-v31/  instruments.json, fx.json, ambience.json (из Data List)
```

Пример сквозняком (темп кита):
```
профиль:  { "id":"kit.common.tempo", "area":"kit", "offset":[0,111], "len":2,
            "encoding":{"nibble":{"div":10}}, "range":{"min":200,"max":2600} }
чтение:   device::address_of(profile,"kit.common.tempo",[kit]) -> [04,kk,00,6F]
          -> sysex::build_rq1(dev, model_id, addr, size) -> байты в MIDI-порт
ответ:    sysex::parse(...) -> DT1{data} -> decode(nibble,/10): 1200/10 = 120.0
          -> Message{param.tempo, 120.0} -> Fluent -> озвучка «темп 120.0»
```
Ни в `sysex`, ни в `device`, ни в `engine` слова «V31» нет — они оперируют профилем.

Рантайм: connect → Identity Reply (Model ID + версия) → registry выбирает
`roland-v31.json` → engine знает ВСЕ адреса/кодировки/каталоги V31 →
читает/пишет/озвучивает. Добавить V51 = положить `roland-v51.json`, **код не трогаем**.

Достаточно ли 2 PDF? Чтобы **построить** профиль — да:
- **MIDI Implementation** → адреса/размеры/кодировки/диапазоны → `areas` + `parameters`.
- **Data List** → каталоги (инструменты/FX/ambience/киты) → `catalogs/*.json`.

Но **проверить на железе** нужно: персистентность (save vs temp, risk #1), точный
формат 4 байт версии прошивки, спорные адреса (темп `0x6F`). PDF строят — железо
подтверждает. Каталог инструментов растёт с Roland Cloud → версионируем/догружаем.

### 3.6 Несколько устройств (экземпляры) и источник правды
**Тип ≠ экземпляр.** Профиль (`roland-v31.json`) — это *тип* модуля. Два одинаковых
V31 дают одинаковый Identity Reply (один Model ID + одна прошивка; серийника в
ответе нет) → по Identity их не различить. **Экземпляр различаем по идентичности
порта от транспорта:** CoreMIDI `kMIDIPropertyUniqueID` (iOS/macOS, стабилен между
переподключениями) / id из Android `MidiDeviceInfo`. Это ключ «какой именно
физический модуль» (см. [ADR-0010](adr/0010-device-instances-and-source-of-truth.md)).

- **Несколько сессий одновременно**, по одной на endpoint. Сессия = endpoint id +
  профиль + **Roland Device ID** (из заголовка Identity Reply) + прошивка + **метка
  пользователя** («Студийный»/«Концертный»), persist по uniqueID. UI различает по
  метке (фолбэк: имя порта / Device ID). В core `Session` уже per-connection.
- Рекомендуем разные **Device ID** (0x10–0x1F) на модулях — родной способ Roland
  различать + нет коллизий адресации, если потоки сольются.

**Источник правды — модуль.** Авторитетной копии не держим; кэш — только для
отзывчивости, всегда сверяется с устройством: активный кит → поллим `Current`;
правки на железе → приходят сами (Transmit Edit Data); заход на кит/параметр →
свежий RQ1. Поэтому изменения «в обход приложения» ловятся by design.

**При старте грузим только текущее состояние** (номер/имя кита, темп — «кит 5,
Jazz»). Полный дамп 200 китов НЕ нужен — лениво, по требованию.

**Инструменты:** *номер* инструмента — состояние (читаем у модуля), *номер↔имя* —
наш каталог. Каталог может отстать от Roland Cloud expansions: неизвестный номер →
«Instrument #N (unknown)», каталог версионируем/догружаем. **Verify on HW:** отдаёт
ли V31 имена инструментов по SysEx (если да — читаем live, расширения «just work»;
обычно Roland имена по MIDI не отдаёт).
...
#[derive(uniffi::Enum)]
pub enum FirmwareSupport { Tested, UntestedNewer, UntestedOlder, Unknown }

#[derive(uniffi::Record)]
pub struct DeviceInfo {
    pub model_id: Vec<u8>,
    pub name: String,
    pub device_id: u8,                   // Roland Device ID (from Identity Reply header)
    pub firmware: Option<String>,        // best-effort display of the 4 version bytes
    pub firmware_support: FirmwareSupport,
    pub profile_id: String,
    pub recognized: bool,                // false => unknown module (degraded mode)
}
// Instance identity (which physical unit) is the transport endpoint id
// (CoreMIDI uniqueID / Android MidiDeviceInfo id) + a user label — both app-side,
// keyed per Session. See ADR-0010.
...
native: bytes → session.handle_midi_input(IdentityReply)   // содержит Model ID + версию ПО
engine: parse → match profile by Model ID → Identifying→Ready
        → fw = FirmwareVersion(reply.version); support = profile.firmware_support(fw)
        → emit DeviceIdentified{device incl. firmware, firmware_support}, Earcon(connected)
        → Speak(High):
            Tested        → "Подключено: {name}, прошивка {fw}."
            Untested*/Unknown → "Подключено: {name}, прошивка {fw}. Эта прошивка не из
                                 протестированных с Tactus — должно работать; сообщите
                                 о проблемах."   (НЕ блокируем — работаем дальше)
...
- **Выбор языка:** по умолчанию — локаль устройства; override в настройках
  (`session.set_locale`).
- **Доступность:** язык анонсов = язык приложения; голос/скорость/подробность —
  настройки скринридера пользователя (применяются автоматически, ADR-0014). RTL
  (ar/he) учитываем в визуальном слое, хотя он вторичен.
...
- **Offline-first:** профили и каталоги встроены; озвучка — системный скринридер;
  сети не требуется.
...
| **Только USB** ([ADR-0015](adr/0015-usb-midi-only.md)) | надёжность, нет пэйринга и запроса разрешений; меньше объём работ | нужен кабель USB-C→USB-B (на Lightning — ещё и переходник) и режим GENERIC на модуле |
...
6. **Apple end-to-end (MVP, сделано первым):** транспорт + анонсы скринридера +
   доступный UI вокруг core; VoiceOver-проход «вслепую».
7. **Android end-to-end:** тот же core; анонсы TalkBack; полный TalkBack-проход.
...
- **Формат 4 байт версии прошивки** в Identity Reply (`version_format`) и **реальная
  версия V31** — снять на HW и внести в `firmware.tested` профиля (см. §3.4).
- **WWDC 2026** — после GA iOS 27/Xcode 27 пересмотреть min-таргеты и API.
- ✅ **Имя продукта** — Tactus (решено).
